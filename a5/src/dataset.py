...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        len_masked_content= np.random.normal(int(len_doc/4), 0.25)
        if len_masked_content < 0:
            len_masked_content= 0
        elif len_masked_content > len_doc:
            len_masked_content= len_doc
        else:
            len_masked_content= int(len_masked_content)
        prefix_end_loc= random.randint(0, len_doc-len_masked_content) #indices
        suffix_start_loc= prefix_end_loc+len_masked_content
        #three substrings
        prefix= document[:prefix_end_loc]
        masked_content= document[prefix_end_loc:suffix_start_loc]
        suffix= document[suffix_start_loc:]
        #rearranging strings
        masked_string= prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        num_pads= self.block_size-len(masked_string)
        masked_string += (self.PAD_CHAR * num_pads)
        #construct inp-out pair
        x= masked_string[:-1]
        y= masked_string[1:]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long) #encoding as Long tensor
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
        return (x,y)
